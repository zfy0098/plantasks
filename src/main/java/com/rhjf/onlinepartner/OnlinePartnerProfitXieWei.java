...
public class OnlinePartnerProfitXieWei extends BaseDao {
	...
	static {
		...
		partnerRate.put("name", "谢伟");
		partnerRate.put("ratio", 0.8);
		partnerRate.put("wxt0", 2.5);
		partnerRate.put("wxt1", 2.4);
		partnerRate.put("alipayt0", 2.5);
		partnerRate.put("alipayt1", 2.4);
		partnerRate.put("qqt0", 2.5);
		partnerRate.put("qqt1", 2.4);
		...
		json.put("id", "167");
		json.put("agentName", "北京鹰博科技有限公司");
		json.put("wxt0", 2.5);
		json.put("wxt1", 2.4);
		json.put("alipayt0", 2.5);
		json.put("alipayt1", 2.4);
		json.put("qqt0", 2.8);
		json.put("qqt1", 2.8);
		json.put("kuait0", 3.3);
		json.put("kuait1", 3.3);
		...
		json = new JSONObject();
		json.put("id", "170");
		json.put("agentName", "浙江汉尼网络科技有限公司");
		json.put("wxt0", 2.5);
		json.put("wxt1", 2.4);
		json.put("alipayt0", 2.5);
		json.put("alipayt1", 2.4);
...
//				String createDate = agentInfo.get("createDate");
				...
				for (int i = 0; i < dateList.size(); i++) {
					...
					String wxProfit = "select bankid , ifnull(sum(profit) , 0) as profit from"
							...
							+ " and date(tpm.createDate)<=date('"+dateList.get(i)+"') and too.orderStatus='SUCCESS' and bankId='WX') as a ";
					...
					String aliPayProfit = "select bankid , ifnull(sum(profit) , 0) as profit from"
							...
							+ " from tbl_online_order as too INNER JOIN tbl_pay_merchant as tpm on too.merchantNo=tpm.merchantNo"
							+ " where tpm.agent_id=? and date(too.createDate)=? "
							+ " and date(tpm.createDate)<=date('"+dateList.get(i)+"')  and too.orderStatus='SUCCESS' and bankId='Alipay') as a ";
					...
					String QQProfit = "select bankid , ifnull(sum(profit) , 0) as profit from"
							...
							+ " from tbl_online_order as too INNER JOIN tbl_pay_merchant as tpm on too.merchantNo=tpm.merchantNo"
							+ " where tpm.agent_id=? and date(too.createDate)=?  "
							+ " and date(tpm.createDate)<=date('"+dateList.get(i)+"')  and too.orderStatus='SUCCESS' and bankId='QQ') as a ";
					...
					String kuaiProfit = "select bankid , ifnull(sum(profit) , 0) as profit from"
							...
							+ " from tbl_online_order as too INNER JOIN tbl_pay_merchant as tpm on too.merchantNo=tpm.merchantNo"
							+ " where tpm.agent_id=? and date(too.createDate)=?  "
							+ " and date(tpm.createDate)<=date('"+dateList.get(i)+"')  and too.orderStatus='SUCCESS' and bankId='KUAI') as a ";
					...
					profitAmount = Double.parseDouble(profitMap.get("profit"));
					profitAmount = profitAmount * radio;
					list.add(new Object[]{dateList.get(i) ,df.format(profitAmount) , "快捷"});
		...
		createExcel2(title, proftlist, sheetName ,pathName, "谢伟");
	}
	...
	public static void main(String[] args) throws Exception {
		OnlinePartnerProfitXieWei onlinePartnerProfit = new OnlinePartnerProfitXieWei();
