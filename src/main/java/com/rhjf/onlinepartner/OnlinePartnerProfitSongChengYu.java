...
public class OnlinePartnerProfitSongChengYu extends BaseDao {
	...
	static {
		...
		partnerRate.put("name", "宋城宇");
		partnerRate.put("ratio", 1);
		partnerRate.put("nett0", 3.5);
		partnerRate.put("nett1", 3.5);
...
//				String createDate = agentInfo.get("createDate");
				...
				for (int i = 0; i < dateList.size(); i++) {
					String wxProfit = "select bankid , ifnull(sum(profit) , 0) as profit from"
							+ " (select too.orderAmount , too.merchantNo , t0PayResult , case WHEN t0PayResult=1 then  ROUND(too.orderAmount*("+agentJSON.getDouble("nett0")+"-"+partnerRate.getDouble("nett0")+")/1000,2) "
							+ " when t0PayResult=0  then ROUND(too.orderAmount*("+agentJSON.getDouble("nett1")+"-"+partnerRate.getDouble("nett1")+")/1000,2) end  as profit , bankid "
							+ " from tbl_online_order as too INNER JOIN tbl_pay_merchant as tpm on too.merchantNo=tpm.merchantNo"
							+ " where tpm.agent_id='"+agentInfo.get("id")+"' and date(too.createDate)=date('"+dateList.get(i)+"') and too.orderStatus='SUCCESS' and b2bOrB2c='NET') as a ";
					...
					list.add(new Object[]{dateList.get(i) ,df.format(profitAmount) , "网银支付"});
		...
		String[] title = {"日期" , "分润金额" , "支付类型"};
		createExcel2(title, proftlist, sheetName ,pathName, "宋城宇");
	}
	...
	public static void main(String[] args) throws Exception {
		OnlinePartnerProfitSongChengYu onlinePartnerProfit = new OnlinePartnerProfitSongChengYu();
