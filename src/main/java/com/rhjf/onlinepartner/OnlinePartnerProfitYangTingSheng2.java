...
public class OnlinePartnerProfitYangTingSheng2 extends BaseDao {
	...
	public void init() throws Exception {
		...
		for (int j = 0; j < array.size(); j++) {
			...
			for (int k = 0; k < agentArray.size(); k++) {
				...
				for (int i = 0; i < dateList.size(); i++) {
					...
					String wxt1Profit = "select  ROUND(orderAmount*("+agentJSON.getDouble("wxt1")+"-"+partnerRate.getDouble("wxt1")+")/1000,2) as profit from"
							+ " (select ifnull(sum(too.orderAmount) , '0') as orderAmount , too.merchantNo , t0PayResult  "
							...
							+ " and date(tpm.createDate)<=date('"+dateList.get(i)+"') and too.orderStatus='SUCCESS' and bankId='WX' and  t0PayResult=0) as a ";
					
					log.info(wxt1Profit);
					
					Map<String,String> profitMap = queryForMapStr(wxt1Profit, null); 
					
					profitAmount += Double.parseDouble(profitMap.get("profit"));
					
					
					String wxt0Profit = "select  ROUND(orderAmount*("+agentJSON.getDouble("wxt0")+"-"+partnerRate.getDouble("wxt0")+")/1000,2) as profit from"
							+ " (select ifnull(sum(too.orderAmount) , '0') as orderAmount , too.merchantNo , t0PayResult  "
							+ " from tbl_online_order as too INNER JOIN tbl_pay_merchant as tpm on too.merchantNo=tpm.merchantNo"
							+ " where tpm.agent_id='"+agentInfo.get("id")+"' and date(too.createDate)=date('"+dateList.get(i)+"')  "
							+ " and date(tpm.createDate)<=date('"+dateList.get(i)+"') and too.orderStatus='SUCCESS' and bankId='WX' and  t0PayResult=1) as a ";
					
					profitMap = queryForMapStr(wxt0Profit, null); 
					...
					String aliPayt0Profit = "select ROUND(orderAmount*("+agentJSON.getDouble("alipayt0")+"-"+partnerRate.getDouble("alipayt0")+")/1000,2) as profit from"
							+ " (select ifnull(sum(too.orderAmount) , '0') as orderAmount , too.merchantNo , t0PayResult  "
							+ " from tbl_online_order as too INNER JOIN tbl_pay_merchant as tpm on too.merchantNo=tpm.merchantNo"
							+ " where tpm.agent_id='"+agentInfo.get("id")+"' and date(too.createDate)=date('"+dateList.get(i)+"')  "
							+ " and date(tpm.createDate)<=date('"+dateList.get(i)+"') and too.orderStatus='SUCCESS' and bankId='Alipay' and  t0PayResult=1) as a ";
					
					profitMap = queryForMapStr(aliPayt0Profit, null); 
					...
					String aliPayt1Profit = "select ROUND(orderAmount*("+agentJSON.getDouble("alipayt1")+"-"+partnerRate.getDouble("alipayt1")+")/1000,2) as profit from"
							+ " (select ifnull(sum(too.orderAmount) , '0') as orderAmount , too.merchantNo , t0PayResult  "
							+ " from tbl_online_order as too INNER JOIN tbl_pay_merchant as tpm on too.merchantNo=tpm.merchantNo"
							+ " where tpm.agent_id='"+agentInfo.get("id")+"' and date(too.createDate)=date('"+dateList.get(i)+"')  "
							+ " and date(tpm.createDate)<=date('"+dateList.get(i)+"') and too.orderStatus='SUCCESS' and bankId='Alipay' and  t0PayResult=0) as a ";
					
					profitMap = queryForMapStr(aliPayt1Profit, null); 
					
					profitAmount += Double.parseDouble(profitMap.get("profit"));
					
					
					log.info(aliPayt1Profit + " : " + profitAmount); 
					
					
					String kuait1Profit = "select ROUND(orderAmount*("+agentJSON.getDouble("kuait1")+"-"+partnerRate.getDouble("kuait1")+")/1000,2) as profit from"
							+ " (select ifnull(sum(too.orderAmount) , '0') as orderAmount , too.merchantNo , t0PayResult  "
							+ " from tbl_online_order as too INNER JOIN tbl_pay_merchant as tpm on too.merchantNo=tpm.merchantNo"
							+ " where tpm.agent_id='"+agentInfo.get("id")+"' and date(too.createDate)=date('"+dateList.get(i)+"')  "
							+ " and date(tpm.createDate)<=date('"+dateList.get(i)+"') and too.orderStatus='SUCCESS' and bankId='KUAI' and  t0PayResult=0) as a ";
					
					profitMap = queryForMapStr(kuait1Profit, null); 
					
					profitAmount += Double.parseDouble(profitMap.get("profit"));
					
					

					String kuait0Profit = "select ROUND(orderAmount*("+agentJSON.getDouble("kuait0")+"-"+partnerRate.getDouble("kuait0")+")/1000,2) as profit from"
							+ " (select ifnull(sum(too.orderAmount) , '0') as orderAmount , too.merchantNo , t0PayResult  "
							+ " from tbl_online_order as too INNER JOIN tbl_pay_merchant as tpm on too.merchantNo=tpm.merchantNo"
							+ " where tpm.agent_id='"+agentInfo.get("id")+"' and date(too.createDate)=date('"+dateList.get(i)+"')  "
							+ " and date(tpm.createDate)<=date('"+dateList.get(i)+"') and too.orderStatus='SUCCESS' and bankId='KUAI' and  t0PayResult=1) as a ";
					
					profitMap = queryForMapStr(kuait0Profit, null); 
		...
		createExcel2(title, proftlist, sheetName ,pathName, "速域（上海）资产管理股份有限公司");
	}
	...
	public static void main(String[] args) throws Exception {
		OnlinePartnerProfitYangTingSheng2 onlinePartnerProfit = new OnlinePartnerProfitYangTingSheng2();
