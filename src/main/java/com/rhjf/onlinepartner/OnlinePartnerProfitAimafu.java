...
public class OnlinePartnerProfitAimafu extends BaseDao {
	...
	static {
		...
		json.put("id", "1519130E-B633-49AC-B62C-8F099C917FA1");
		json.put("agentName", "周小丽");
		json.put("wxt0", 2.9);
		json.put("wxt1", 2.9);
		json.put("alipayt0", 2.9);
		json.put("alipayt1", 2.9);
		json.put("qqt0", 2.9);
		json.put("qqt1", 2.9);
		json.put("kuait0", 2.9);
		json.put("kuait1", 2.9);
...
//							+ " where tpm.agent_id='"+agentInfo.get("id")+"' and date(too.createDate)=date('"+dateList.get(i)+"') and too.orderStatus='SUCCESS' and bankId='WX') as a ";
					...
					String wxProfit = "select ifnull(sum(profit) , 0) as profit from "
							+ "(select tpo.amount , tpo.TradeCode , case when tpo.TradeCode='T0' then ROUND(tpo.Amount*("+agentJSON.getDouble("wxt0")+"-"+partnerRate.getDouble("wxt0")+")/1000/100,2) "
							+ " when tpo.TradeCode='T1' then ROUND(tpo.Amount*("+agentJSON.getDouble("wxt1")+"-"+partnerRate.getDouble("wxt1")+")/1000/100,2) end as profit"
							+ " from tab_pay_order as tpo INNER JOIN tab_loginuser as tl on tpo.UserID=tl.ID"
							+ " where tl.AgentID='" + agentInfo.get("ID") + "' and tpo.LocalDate='"+dateList.get(i)+"'  and tpo.PayRetCode='0000' "
							+ " and (PayChannel=1 or PayChannel=2 or PayChannel=3)) as a ";
					...
					String kuaiProfit = "select ifnull(sum(profit) , 0) as profit from "
							+ "(select tpo.amount , tpo.TradeCode , case when tpo.TradeCode='T0' then ROUND(tpo.Amount*("+agentJSON.getDouble("kuait0")+"-"+partnerRate.getDouble("kuait0")+")/1000/100,2)"
							+ " when tpo.TradeCode='T1' then ROUND(tpo.Amount*("+agentJSON.getDouble("kuait1")+"-"+partnerRate.getDouble("kuait1")+")/1000/100,2) end as profit"
							+ " from tab_pay_order as tpo INNER JOIN tab_loginuser as tl on tpo.UserID=tl.ID"
							+ " where tl.AgentID='" + agentInfo.get("ID") + "' and tpo.LocalDate='"+dateList.get(i)+"'  and tpo.PayRetCode='0000' "
							+ " and (PayChannel=4)) as a ";
					
					
					profitMap = queryForMapStr(kuaiProfit, null); 
	...
	public static void main(String[] args) throws Exception {
		OnlinePartnerProfitAimafu onlinePartnerProfit = new OnlinePartnerProfitAimafu();
